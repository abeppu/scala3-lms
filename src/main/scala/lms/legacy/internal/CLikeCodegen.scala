...
trait CLikeCodegen extends GenericCodegen {
  ...
  val dsTypesList = HashSet[(Typ[?],String)]()

  // Streams for helper functions and its header
  var helperFuncStream: PrintWriter = uninitialized
  var headerStream: PrintWriter = uninitialized
  var actRecordStream: PrintWriter = uninitialized
  var typesStream: PrintWriter = uninitialized
  ...
  def emitValDef(sym: String, tpe: Typ[?], rhs: String): Unit = {
  ...
  override def remap[A](m: Typ[A]) : String = m match {
    ...
    case _ if m.runtimeClass == classOf[List[?]] => // Use case: Delite Foreach sync list
