...
trait ImplicitOps extends Base {
   ...
   **/
  def implicit_convert[X,Y](x: Rep[X])(using c: X => Y, mX: Typ[X], mY: Typ[Y], pos: SourceContext) : Rep[Y] // = x.asInstanceOf[Rep[Y]
}

trait ImplicitOpsExp extends ImplicitOps with BaseExp {
  case class ImplicitConvert[X,Y](x: Exp[X])(using val mX: Typ[X], val mY: Typ[Y]) extends Def[Y]

  def implicit_convert[X,Y](x: Exp[X])(using c: X => Y, mX: Typ[X], mY: Typ[Y], pos: SourceContext) : Rep[Y] = {
  ...
  override def mirror[A:Typ](e: Def[A], f: Transformer)(using pos: SourceContext): Exp[A] = (e match {
    case im@ImplicitConvert(x) => toAtom(ImplicitConvert(f(x))(using im.mX,im.mY))(using mtyp1[A],pos)
