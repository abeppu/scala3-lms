...
- [x] Restore in-process eval and non-Scala backends (C/CUDA/OpenCL)
  ...
  - [x] Bring up minimal C backend smoke coverage before attempting full query codegen
    ...
    - [x] Add source-only CUDA smoke coverage for the existing `CudaCodegen.emitSource` path so `.cu`-style output remains testable without `nvcc`
    - [x] Keep CUDA compile/run and OpenCL deferred until a CUDA/OpenCL toolchain is explicitly in scope; this container has no `nvcc`, `clang`, or `clinfo`
